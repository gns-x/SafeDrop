...
const mockPickups = Array.from({ length: 20 }, (_, i) => ({
  ...
  location: ["Main Gate", "Side Entrance", "Parking Lot"][
...
const PickupsPage = () => {
  ...
  const filteredPickups = mockPickups.filter((pickup) => {
    const matchesSearch =
      pickup.studentName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pickup.parentName.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
      statusFilter === "all" || pickup.status === statusFilter;
    const matchesLocation =
      locationFilter === "all" || pickup.location === locationFilter;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-gray-600">
              Monitor and manage student pickup requests
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {[
              ...
              {
                label: "Pending",
                value: mockPickups.filter((p) => p.status === "pending").length,
                color: "yellow",
              },
              {
                label: "In Progress",
                value: mockPickups.filter((p) => p.status === "in-progress")
                  .length,
                color: "blue",
              },
              {
                label: "Completed",
                value: mockPickups.filter((p) => p.status === "completed")
                  .length,
                color: "green",
              },
            ].map((stat) => (
              ...
              >
                <p className={`text-${stat.color}-600 text-sm font-medium`}>
                  {stat.label}
                </p>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredPickups.map((pickup) => (
                    <tr key={pickup.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(pickup.status)}`}
                        >
                          {pickup.status}
                        </span>
                      </td>
