...
export const RecentActivityFeed: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'user' | 'pickup' | 'payment' | 'system' | 'security'>('all');
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-xl text-gray-900">Recent Activity Feed</CardTitle>
                <p className="text-gray-600">Live system events and user activities</p>
              </div>
            </div>
              ...
              {filteredActivities.map((activity, index) => (
                ...
                >
                  <div className="flex items-start space-x-4 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-white/20 hover:shadow-md transition-all duration-200 hover:scale-[1.02]">
                    {/* Icon */}
                    <div className={`p-3 rounded-xl bg-gradient-to-br ${getTypeColor(activity.type)} shadow-lg`}>
